...
for ($y = 0; $y -lt $h; $y++) {
    for ($x = 0; $x -lt $w; $x++) {
        ...
        if ($c.A -gt 0) {
            ...
            # Reduce the previous 1.035 multiplier by another 4% (1.035 * 0.96 = 0.9936)
            $multiplier = $multiplier * 0.9936
...
$bmp.Dispose()
$newBmp.Dispose()
Write-Host 'Done shrinking eyeshadow (final lighter version)'
