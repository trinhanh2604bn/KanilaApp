...
import com.example.frontend.feature.arcore.rendering.ShaderUtil.loadGLShader
...
public class AugmentedFaceRenderer {
    ...
    private val tintColor = floatArrayOf(0f, 0f, 0f, 0f)

    fun setTintColor(color: FloatArray) {
        if (color.size >= 4) {
            System.arraycopy(color, 0, tintColor, 0, 4)
        }
    }
    ...
    fun draw(
        ...
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureId[0])
        GLES20.glUniform4f(tintColorUniform, tintColor[0], tintColor[1], tintColor[2], tintColor[3])
